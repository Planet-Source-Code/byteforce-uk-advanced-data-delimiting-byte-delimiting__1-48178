...
'// frmClient
'// *********
'// The client side of the connection. Makes a connection to a server on port
'// 5678.
'//
'// ©2003 Matthew Hall
...
snd_data:
    ...
    If Len(vTCPSendBuffer) > 4096 Then
        ...
        DoEvents
        wsClient.SendData Mid(vTCPSendBuffer, 1, 4096)
        vTCPSendBuffer = Mid(vTCPSendBuffer, 4096)
        GoTo snd_data
        
    Else
        
        wsClient.SendData vTCPSendBuffer
...
Private Sub cmdListen_Click()
    ...
    wsClient.Close
    wsClient.Connect "localhost", "5678"
    
    txtOutput.Text = txtOutput.Text & "socket connecting.." & vbCrLf
